...
@section('title', 'Edit Produk Hukum - JDIH Modern')
...
<div class="mb-10 flex items-center justify-between">
    <div>
        ...
        <h2 class="text-3xl font-black text-slate-900">Perbarui Produk Hukum</h2>
        <p class="text-slate-500 text-sm">Sesuaikan rincian 10 Dimensi Klasterisasi & Konten.</p>
    </div>
    <div class="flex items-center gap-4">
        <button type="button" onclick="document.getElementById('main-form').submit()" style="background-color: #0c1120;" class="px-8 py-4 text-white rounded-2xl font-black uppercase tracking-widest text-xs flex items-center gap-3 hover:opacity-95 shadow-xl transition-all">
            <i data-lucide="save" class="w-5 h-5 text-islami"></i>
            Simpan Perubahan
        </button>
    </div>
</div>
...
<form id="main-form" action="{{ route('admin.documents.update', $document) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div id="tab-info" class="tab-content space-y-8">
        <div class="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-8">
            <div>
                ...
                <textarea name="title" rows="3" required class="w-full px-6 py-5 bg-slate-50 border-slate-200 rounded-3xl font-bold text-lg focus:ring-0 focus:border-islami transition-all">{{ old('title', $document->title) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                <div>
                    ...
                    <input type="text" name="number" required value="{{ old('number', $document->number) }}" class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl font-bold focus:ring-0 focus:border-islami">
                </div>
                <div>
                    <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-3">Tahun Terbit</label>
                    <input type="number" name="year" required value="{{ old('year', $document->year) }}" class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl font-bold focus:ring-0 focus:border-islami">
                </div>
                <div>
                    <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-3">Tgl. Penetapan</label>
                    <input type="date" name="tanggal_penetapan" value="{{ old('tanggal_penetapan', $document->tanggal_penetapan?->format('Y-m-d')) }}" class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl font-bold focus:ring-0 focus:border-islami">
                </div>
                <div>
                    <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-3">Tgl. Pengundangan</label>
                    <input type="date" name="tanggal_pengundangan" value="{{ old('tanggal_pengundangan', $document->tanggal_pengundangan?->format('Y-m-d')) }}" class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl font-bold focus:ring-0 focus:border-islami">
                </div>
            </div>

            <div>
                <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-3">Abstrak / Ringkasan</label>
                <textarea name="abstrak" rows="4" class="w-full px-6 py-5 bg-slate-50 border-slate-200 rounded-3xl focus:ring-0 focus:border-islami">{{ old('abstrak', $document->abstrak) }}</textarea>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-10">
            <div>
                <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-4 flex items-center gap-2">
                    <i data-lucide="layers" class="w-4 h-4"></i> Jenis Produk Hukum
                </label>
                <select name="category_id" required class="w-full px-6 py-5 bg-slate-50 border-slate-200 rounded-3xl font-bold focus:ring-0 focus:border-islami appearance-none">
            ...
            <div>
                ...
                <select name="subject_id" class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl text-sm font-bold focus:ring-0 focus:border-[#0F9D58]">
                    <option value="">-- Semua Bidang --</option>
                    @foreach($subjects as $sub)
                        <option value="{{ $sub->id }}" {{ $document->subject_id == $sub->id ? 'selected' : '' }}>{{ $sub->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="territory_id" class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl text-sm font-bold focus:ring-0 focus:border-[#0F9D58]">
                    <option value="">-- Semua Wilayah --</option>
                    @foreach($territories as $ter)
                        <option value="{{ $ter->id }}" {{ $document->territory_id == $ter->id ? 'selected' : '' }}>{{ $ter->name }}</option>
                    @endforeach
                </select>
                <input type="hidden" name="territory_level" value="{{ $document->territory_level->value }}">
            </div>

            <!-- Document Function -->
            <div class="md:col-span-2">
                <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-3">Fungsi Dokumen</label>
                <select name="function_id" class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl text-sm font-bold focus:ring-0 focus:border-[#0F9D58]">
                    <option value="">-- Semua Fungsi --</option>
                    @foreach($functions as $fn)
                        <option value="{{ $fn->id }}" {{ $document->function_id == $fn->id ? 'selected' : '' }}>{{ $fn->name }}</option>
                    @endforeach
                </select>
                <input type="hidden" name="document_function" value="{{ $document->document_function->value }}">
            </div>
        </div>
    </div>
    ...
    <div id="tab-konten" class="tab-content hidden space-y-8">
        <div class="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
            ...
            <textarea name="content_html" rows="20" class="w-full px-6 py-5 bg-slate-50 border-slate-200 rounded-3xl focus:ring-0 focus:border-islami">{{ old('content_html', $document->content_html) }}</textarea>
        </div>
    </div>
    ...
    <div id="tab-pengaturan" class="tab-content hidden grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
                ...
                <div id="relations-container" class="space-y-4">
                     @foreach($document->relations as $index => $rel)
                        <div id="relation-row-init-{{ $index }}" class="p-6 bg-slate-50/50 rounded-2xl border border-slate-200 space-y-4 group hover:bg-white transition-all">
                            <div class="space-y-4">
                                <select name="relations[{{ $index }}][related_document_id]" class="w-full jdin-select2" required>
                                    @foreach($allDocuments as $doc)
                                        <option value="{{ $doc->id }}" {{ $rel->related_document_id == $doc->id ? 'selected' : '' }}>{{ strip_tags($doc->title) }} ({{ $doc->year }})</option>
                                    @endforeach
                                </select>
                                <div class="flex items-center gap-4">
                                    <select name="relations[{{ $index }}][relation_type]" class="flex-1 px-4 py-2 border-slate-200 rounded-xl text-xs font-bold" required>
                                         @foreach(\App\Enums\RelationType::cases() as $rt)
                                            <option value="{{ $rt->value }}" {{ $rel->relation_type == $rt ? 'selected' : '' }}>{{ $rt->label() }}</option>
                                         @endforeach
                                    </select>
                                    <button type="button" onclick="document.getElementById('relation-row-init-{{ $index }}').remove()" class="p-2 text-red-400 hover:text-red-500 transition-all">
            ...
            <div class="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
                ...
                <select name="tags[]" multiple class="w-full jdin-select2-tags">
                    @foreach($tags as $tag)
                        <option value="{{ $tag->id }}" {{ in_array($tag->id, $document->tags->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $tag->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="space-y-8">
                ...
                <div>
                     ...
                     <select name="legal_status" required class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl font-bold text-sm">
                         @foreach(\App\Enums\LegalStatus::cases() as $ls)
                             <option value="{{ $ls->value }}" {{ $document->legal_status == $ls ? 'selected' : '' }}>{{ $ls->label() }}</option>
                         @endforeach
                     </select>
                </div>
                ...
                <div>
                     ...
                     <select name="access_level" required class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl font-bold text-sm">
                         @foreach(\App\Enums\AccessLevel::cases() as $al)
                             <option value="{{ $al->value }}" {{ $document->access_level == $al ? 'selected' : '' }}>{{ $al->label() }}</option>
                         @endforeach
                     </select>
                </div>
                ...
                <div>
                     ...
                     <select name="status" required class="w-full px-5 py-4 bg-slate-50 border-slate-200 rounded-2xl font-bold text-sm">
                         @foreach(\App\Enums\DocumentStatus::cases() as $ds)
                             <option value="{{ $ds->value }}" {{ $document->status == $ds ? 'selected' : '' }}>{{ $ds->label() }}</option>
                         @endforeach
                     </select>
                </div>
            </div>
            ...
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-6 uppercase tracking-widest">Berkas PDF</label>
                
                <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200 mb-6 flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl bg-islami/10 text-islami flex items-center justify-center">
                        <i data-lucide="file-text" class="w-6 h-6"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-[9px] font-black uppercase text-slate-400 leading-none mb-1">Current Berkas</p>
                        <p class="text-[11px] font-bold truncate">{{ basename($document->file_path) }}</p>
                    </div>
                </div>
                ...
                <div class="relative group">
                    <div id="file-drop-area" class="bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl p-10 flex flex-col items-center justify-center text-slate-400 group-hover:border-islami transition-all">
                        ...
                        <p id="file-name-display" class="text-[9px] font-bold uppercase text-center">Ganti PDF (Optional)</p>
                    </div>
                    <input type="file" name="file" id="file-input" accept=".pdf" class="absolute inset-0 opacity-0 cursor-pointer">
                </div>
...
<script>
    ...
    $('.jdin-select2').select2({ placeholder: "Cari Produk Hukum...", width: '100%' });
    ...
    document.getElementById('file-input').addEventListener('change', function(e) {
        ...
        if (e.target.files.length > 0) {
            ...
            fileArea.innerHTML = `
                ...
                <button type="button" onclick="resetEditFileInput()" class="mt-4 text-[9px] font-black uppercase text-red-400 hover:text-red-500 underline tracking-widest">Batalkan Perubahan</button>
            `;
    ...
    // Relation Dynamics
    let relationIndex = {{ count($document->relations) + 100 }};
    ...
    function addRelationRow() {
        ...
        rowBlock.innerHTML = `
            <div class="space-y-4">
                <select name="relations[${relationIndex}][related_document_id]" class="w-full jdin-select2-dynamic" required>
                    ${options}
                </select>
                <div class="flex items-center gap-4">
        ...
        $(`#${rowId} .jdin-select2-dynamic`).select2({ 
